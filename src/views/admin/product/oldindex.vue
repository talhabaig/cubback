<template>
  <div>
      ...
      <v-card-actions class="pa-5">
        ...
        >
          <template v-slot:activator="{ on, attrs }">
            <v-btn v-if="hasPermission('product_create')" dark color="main_bg_color" v-bind="attrs" v-on="on">
    ...
    <v-card v-if="hasPermission('product_view')">
      <v-card-title>
        Product List
        <v-spacer></v-spacer>
        <v-text-field
      ...
      <v-data-table :headers="headers" :items="items" :search="search">
        ...
        <template  v-slot:[`item.actions`]="{ item }">
          <v-icon v-if="hasPermission('product_edit')" class="mr-2 edit_btn" @click.stop="editItem(item)">
            mdi-pencil
          </v-icon>
          <v-icon v-if="hasPermission('product_delete')" @click="warningModel(item._id)" class="del_btn">
            mdi-delete
          </v-icon>
        </template>
      </v-data-table>
...
export default {
  ...
  computed: {
    ...mapGetters(["getproductList", "userDetails"]),
  },
};
</script>

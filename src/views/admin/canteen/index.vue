          ...
          <template v-if="dialog" v-slot:default="dialog">
            <v-card>
              <v-toolbar color="main_bg_color" dark class="mb-2">
                ...
                >
                <span v-else><i class="fas fa-stream Orders mr-2"></i>Edit Canteen</span
                >
              </v-toolbar>
              <v-card-text>
...
export default {
    ...
    return {
      ...
      headers: [
        ...
        },
        {
          ...
          align: "start",
          sortable: true,
          value: "canteen_location",
        },
        {
          text: "Status",
          align: "start",
          sortable: true,
          value: "canteen_status",
        },
        {
          value: "actions",
          text: "Action",
  ...
  methods: {
   ...
   async editItem(obj) {  
     await this.$store.dispatch("getcanteenById", obj);
      this.dialog = true;
      this.isEdit = true;
    },
    ...
    async deleteItem() {
     await this.$store.dispatch("removecanteen", this.itemId)
      .then((res) => {
              Swal.fire({
                title: "",
